...
# mlp,可以发现有四个，对应4个stag，每一层会循环多次，循环时会将当前层强转为字符
expansion_ratios_L = {
    '0': [4, 4, 4, 4, 4],
    '1': [4, 4, 4, 4, 4],
...
def stem(in_chal, out_chal, act_layer=nn.ReLU):
    # 类似下采样
    return nn.Sequential(
        nn.Conv2d(in_chal, out_chal // 2, kernel_size=3, stride=2, padding=1),
        nn.BatchNorm2d(out_chal // 2),
        act_layer(),
        nn.Conv2d(out_chal // 2, out_chal, kernel_size=3, stride=2, padding=1),
        nn.BatchNorm2d(out_chal),
        act_layer()
    )
...
class AttnFFN(nn.Module):
    def __init__(self, dim, mlp_ratio=4.,
        ...
        super().__init__()
        # 注意力处理完后进入mlp
        self.att4d = Attention4D(dim, resolution=resolution, act_layer=act_layer, stride=stride)

        hidden = int(dim * mlp_ratio)

        self.mlp = Mlp(in_features=dim, hidden_features=hidden,
    ...
    def forward(self, x):
        if self.use_layer_scale:
            x = x + self.drop_path(self.layer_scale_1 * self.att4d(x))
            x = x + self.drop_path(self.layer_scale_2 * self.mlp(x))
        else:
            x = x + self.drop_path(self.att4d(x))
...
class FFN(nn.Module):
    ...
    def forward(self, x):
        if self.use_layer_scale:
            x = x + self.drop_path(self.layer_scale_1 * self.mlp(x))
        else:
            x = x + self.drop_path(self.mlp(x))
        return x
...
def eformer_block(dim, index, layers,
    ...
    blocks = []
    for b_index in range(layers[index]):
        block_dpr = drop_path_rate * (b_index + sum(layers[:index])) / (sum(layers) - 1)
        mlp_ratio = e_ratios[str(index)][b_index]  # mlp

        if index >= 2 and b_index > layers[index] - 1 - vit_num:
            if index == 2:
                stride = 2
            else:
                stride = None
            ...
            blocks.append(FFN(dim, pool_size=pool_size, mlp_ratio=mlp_ratio,
                              act_layer=act_layer,
                              drop=drop_rate, drop_path=block_dpr,
                              use_layer_scale=use_layer_scale,
                              layer_scale_init_value=layer_scale_init_value, ))
    ...
    def __init__(self, patch_size=3, stride=2, padding=1,
        ...
        if self.light:
            ...
            self.new_proj = nn.Sequential(
                nn.Conv2d(in_chans, in_chans, kernel_size=3, stride=2, padding=1),
            ...
            # 两步依旧独立都进行下采样，实现特征融合
            self.atten = attn_block(dim=in_chans, out_dim=embed_dim,
            ...
            self.b = norm_layer(embed_dim) if norm_layer else nn.Identity()
...
class CCA(nn.Module):
    ...
    def forward(self, g, x):
        p_x = F.avg_pool2d(x, (x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
        c_x = self.mlp_x(p_x)

        p_g = F.avg_pool2d(g, (g.size(2), g.size(3)), stride=(g.size(2), g.size(3)))
        c_g = self.mlp_g(p_g)

        total_chal = (c_g + c_x) / 2.0

        scale = torch.sigmoid(total_chal).unsqueeze(2).unsqueeze(3).expand_as(x)

        x_att_chal = x * scale
        out = self.a(x_att_chal)
        return out


# resolution2为输出维度，在注意力中使用
...
def eformer_block_up(dim, index, layers,
    ...
    for b_index in range(layers[index]):
        ...
        if index <= 1 and b_index > layers[index] - 1 - vit_num:
            if index == 1:
                stride = 2
            else:
                stride = None
